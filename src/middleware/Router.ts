...
export class Router {
    ...
    private catchError(err: any, response: Response) {
    ...
    public handler = (finalHandler: AWSCallback, preMiddlewares: AbstractMiddleware<any>[] = [], postMiddlewares: AbstractMiddleware<any>[] = []) => async (event: LambdaRequest<any>, context: APIGatewayEventRequestContext) => {
    ...
    public classHandler = (classHandler: any, name: string, preMiddlewares: AbstractMiddleware<any>[] = [], postMiddlewares: AbstractMiddleware<any>[] = []) => async (event: LambdaRequest<any>, context: APIGatewayEventRequestContext) => {
        const response = this.preHandle(event, context);
        try {
            const fullMiddlewares = preMiddlewares.concat(this.middlewares, postMiddlewares);
    ...
    public addClass(name: string, handler: any, preMiddlewares?: AbstractMiddleware<any>[], postMiddlewares?: AbstractMiddleware<any>[]) {
        return this.classHandler(handler, name, preMiddlewares, postMiddlewares);
    }

    public static handle(req: APIGatewayProxyEvent, context: APIGatewayEventRequestContext) {
